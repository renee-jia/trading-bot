...
| Column | Rule |
|---|---|
| **建仓** (enter via short put) | `适合` = score ≥ 60, price ≥ 97% of SMA200, no macro blackout, an expiry that clears earnings and FOMC/CPI/NFP/PCE, IV not more than 1pt below trimmed realized vol. `可小量` = score 50–60 (lowest strike, one contract). `权利金薄` = IV below realized vol. `已持有` = held via `CC_POSITIONS`/Alpaca: the put is an add, not a new entry. `不建议` = below SMA200 or score < 50. |
